...
from lightweight_human_pose_estimation.msg import KeyPoint2D #検出された2Dの関節点情報を表す
from lightweight_human_pose_estimation.msg import KeyPoint2DArray #検出された2Dの関節点情報を表す
...
import sys
import getpass
sys.path.append(os.path.join("/home/" + str(getpass.getuser()) + "/colcon_ws/src/lightweight_human_pose_estimation/script"))
...
class Flame(Node) :
    def __init__(self):
    # rospy.get_param()と書かれていたので，ここをROS2化した．↓
        ...
        self.declare_parameters(
            namespace='',
            parameters=[
                ('checkpoint_path', '/home/sobits/colcon_ws/src/lightweight_human_pose_estimation/script/checkpoints/checkpoint_iter_370000.pth'),
                ('height_size', 256),
                ('cpu', True),
                ('track', True),
                ('smooth', True),
